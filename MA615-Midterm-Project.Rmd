...
The first obvious mess thing is that, it is sure not each of columns should be a single variable. Inside columns, there are both variables and observations. For those already listed variables, "CBSA_LABEL" are area information, "NAC3_Label" are industry information; they are for sure fixed variables (dimensions) of the dataframe. For variables "X_TYPE_" and "TOTAL_UNIT", I can't figure out what do they mean; however, counting the outcomes of this variable:
```{r}
count(data$X_TYPE_)
```
there is only one kind of outcome. So I think it is proper to treat it as a fixed variable and leave it unchanged. 
For "TOTAL_UNIT", as long as it seems could be a proper variable, I'll live it and see it as already tidied.
```{r}
